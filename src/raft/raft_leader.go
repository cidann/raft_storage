...
func (rf *Raft) startLeader(term int) {
	...
	rf.state = LEADER
	Debug(rf, dLeader, "S%d is now leader of term %d latest entry: %v", rf.me, rf.currentTerm, rf.log.last())
...
func (rf *Raft) leaderCheckAndUpdateCommit(new_commit_index int) {
	...
	if count > len(rf.peers)/2 {
		Debug(rf, dCommit, "S%d replicated from index %d to %d", rf.me, rf.commitIndex, new_commit_index)
